...
4. **Dependency Injection**: Registering Refit with `HttpClientFactory` (`services.AddRefitClient<ITmdbApi>()`).
5. **Request & Response Body Serialization**: Automatic JSON serialization for request bodies (`[Body] Rating rating`) and response models.
6. **Metadata & Error Inspection with `ApiResponse<T>`**: Using Refit's generic response wrapper to inspect HTTP status codes, error payloads, and response headers without throwing unhandled exceptions.
7. **Interactive API UI**: Powered by `Microsoft.AspNetCore.OpenApi` and `Scalar.AspNetCore` in .NET 10.
